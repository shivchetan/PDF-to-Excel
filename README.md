...
if uploaded is not None:
    ...
    file_hash = _file_sha1(pdf_bytes)
    ...
    do_convert = st.button("Convert to Excel")

    if do_convert:
        cache_key = (file_hash, chosen_method, normalised_pages, include_text)
        if cache_key in st.session_state["cache"]:
            tables, text_pages, method_used, xlsx_bytes = st.session_state["cache"][cache_key]
        else:
            ...
            if len(tables) == 0 and not include_text:
                st.warning(
                    "No tables were detected. Consider enabling the 'Include raw text' option "
            ...
            st.session_state["cache"][cache_key] = (tables, text_pages, method_used, xlsx_bytes)
